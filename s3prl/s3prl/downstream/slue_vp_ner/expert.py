...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, **kwargs):
        ...
        self.dictionary = Dictionary.load(self.datarc.get("dict_path"))
    ...
    def _prep_data_for_ner_eval(self, wrd_lst, sent_data_dct, label_data_dct, pfx, score_type="standard"):
        """
        Read the GT and predicted utterances in the entity format [(word1, tag1), (word2, tag2), ...]
        """
        spl_char_to_entity = eval_utils_ner.spl_char_to_entity
        entity_end_char = "]"
        ...
        label_lst = []
        sent_data_dct[pfx].append(" ".join(wrd_lst))
        ...
        label_data_dct[pfx].append(make_distinct(label_lst))


    def _compute_metrics_asr(self, pred_tokens_all, pred_words_all, target_tokens_all, target_words_all):
    ...
    def _compute_metrics_ner(self, pred_words_all, target_words_all, eval_type="standard"):
        sent_data_dct = {"hypo": [], "ref": []}
        label_data_dct = {"hypo": [], "ref": []}
        for pred_words, target_words in zip(
            pred_words_all, target_words_all):
            self._prep_data_for_ner_eval(pred_words, sent_data_dct, label_data_dct, "hypo", eval_type)
            self._prep_data_for_ner_eval(target_words, sent_data_dct, label_data_dct, "ref", eval_type)
        metrics = eval_utils_ner.get_ner_scores(label_data_dct["ref"], label_data_dct["hypo"])
        return metrics 
    ...
    def log_records(self, split, records, logger, global_step, batch_ids, total_batch_num, **kwargs):
        ...
        uer, wer = self._compute_metrics_asr(
            records['pred_tokens'],
            records['pred_words'],
        ...
        print(f'{split} NER fscore: {fscore}')

        save_names = []
        if split == 'dev' and fscore < self.best_score:
        # if split == 'dev' and wer < self.best_score:
            # self.best_score = torch.ones(1) * wer
            self.best_score = torch.ones(1) * fscore
